    ...
    def interpolate_2d(self, x_values, y_values, z_values, x_points, y_points):
        ...
        for i in range(len(x_points)):
            ...
            if x in x_values and y in y_values:
                ...
                left_x = max([x for x in x_values if x < x])
                right_x = min([x for x in x_values if x > x])
                left_y = max([y for y in y_values if y < y])
                right_y = min([y for y in y_values if y > y])
